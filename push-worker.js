self.host_url = "https://cdn.gravitec.net/";
self.app_key = "9606cb5c537c487388a9ea58109a01af";
self.api_url = "https://api.gravitec.net/";
self.loadScript = false;
self.version = '26.10.2018';
self.functions = {
...
function pushDefault(event) {
  ...
  var promise = self.functions.push.call(self, event);
...
function notificationclickDefault(event) {
  if (typeof self.notificationclick === 'undefined') {
    loadBase();
  }
  if (self.notificationclick) {
    return self.notificationclick(event)
  }
  ...
  return event.waitUntil(promise.then(function (subscription) {
    ...
    return Promise.all(self.functions.pushclick(event, regID));
  }));
}

function notificationCloseDefault(event) {

  if (typeof self.notificationclose === 'undefined') {
    loadBase();
    if (typeof self.notificationclose !== 'undefined') return self.notificationclose(event);
  }

  event.waitUntil(this.registration.pushManager.getSubscription()
    .then(function (subscription) {
      var regID = prepareId(subscription).gid;
      return Promise.all(self.functions.pushclose(event, regID));
    }));
}
...
function activateDefault(event) {
  ...
  event.waitUntil(self.functions.activate(event));
}

function installDefault(event) {
  if (typeof self.install === 'undefined') {
    loadBase();
  }
  if (self.install) {
    return self.install(event);
  }
  sendMessage("Trace worker", 'install', true);
  event.waitUntil(self.functions.install(event));
}
...
function loadBase() {
  try {
    return self.importScripts(self.host_url + 'push/shim/base.js?token=' + new Date().getTime())
  } catch (e) {
...
function loadShim(shim) {
  self.importScripts(self.host_url + 'push/shim/helper.js?token=' + new Date().getTime());
  return new Promise(function (resolve) {
    self.importScripts(self.host_url + 'push/shim/' + shim + '.js?token=' + new Date().getTime());
  });
}
...
function onPushFunc(event) {
  return this.registration.pushManager.getSubscription().then(function (subscription) {
    ...
    function fetchDataFromServer() {
      fetchItteration++;
      return fetch(self.api_url + "api/sites/lastmessage/?regID=" + encodeURIComponent(regID) + "&app_key=" + self.app_key + "&version=" + (self.version || 1))
        .then(function (response) {
          serverResponse = response;
          if (response.status < 200 || response.status >= 300) {
            if (self.logging) console.log('Looks like there was a problem. Status Code: ' + response.status);
          ...
          return response.json().then(function (jsons) {
            if (!jsons || (jsons instanceof Array && !jsons.length)) {
...
function onPushClickFunc(event, regID) {
  ...
  if (event.action) {
    ...
    if (buttonData.url) {
      ...
      promiseRequest = fetch(self.api_url + "api/sites/" + messageid + "/read?regID=" + encodeURIComponent(regID) + "&app_key=" + self.app_key + "&button_id=" + buttonData.id)
    }
    ...
    promiseRequest = fetch(self.api_url + "api/sites/" + messageid + "/read?regID=" + encodeURIComponent(regID) + "&app_key=" + self.app_key + "&version=" + (self.version || 1));
  }
...
function onPushCloseFunc(event, regID) {
  ...
  promiseRequest = fetch(self.api_url + "api/sites/" + messageid + "/close?regID=" + encodeURIComponent(regID) + "&app_key=" + self.app_key + "&version=" + (self.version || 1));
  ...
  try {
    ...
    if (jsons instanceof Array) {
      for (i = 0; i < jsons.length; i++) {
        fetch(self.api_url + "api/sites/receive/" + jsons[i].id + "?regID=" + encodeURIComponent(regID) + "&app_key=" + self.app_key + "&version=" + (self.version || 1)).catch(function (err) {
    ...
    fetch(self.api_url + "api/sites/receive/" + jsons.id + "?regID=" + encodeURIComponent(regID) + "&app_key=" + self.app_key + "&version=" + (self.version || 1)).catch(function (err) {
      sendMessage("Error fetching receive", err, false, regID);
    });
...
function showNotification(json, regID) {
  ...
  notifOptions.icon = (json.icon || self.host_url + 'img/icons/Icon-Message.png') + '?' + generateQueryString(notifData);
...
function sendMessage(subject, error, isTrace, token) {
  ...
  return promise.then(function (token) {
    var errorUrl = self.api_url + 'api/sites/' + (isTrace ? 'logtraceworker' : 'logworkerrors') + '/?app_key=' + self.app_key + '&token=' + token + '&time=' + Math.floor(Date.now() / 1000) + "&version=" + (self.version || 1);
...
function clientActions(url, action) {
  if (url) {
    if (action === 'open' && self.clients.openWindow) {
